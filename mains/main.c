...
// 사용자 Custom FFT 영역
// -----------------------------------------------------------------------------
void custom_fft_2048_fixed(float* real, float* imag) {
    int j_idx = 0; for (int i = 0; i < 2047; i++) { if (i < j_idx) { float tr = real[i]; real[i] = real[j_idx]; real[j_idx] = tr; float ti = imag[i]; imag[i] = imag[j_idx]; imag[j_idx] = ti; } int k = 1024; while (k <= j_idx) { j_idx -= k; k /= 2; } j_idx += k; }
    for (int step = 1; step < 2048; step *= 2) { const int jump = step * 2; const int twiddle_step = 2048 / jump;
        if (step < 4) { for (int i = 0; i < 2048; i += jump) { for (int j = 0; j < step; j++) { int curr = i + j; int k = curr + step; float tr = twiddle_real_2048[j * twiddle_step]; float ti = twiddle_imag_2048[j * twiddle_step]; float t_real = real[k] * tr - imag[k] * ti; float t_imag = real[k] * ti + imag[k] * tr; real[k] = real[curr] - t_real; imag[k] = imag[curr] - t_imag; real[curr] = real[curr] + t_real; imag[curr] = imag[curr] + t_imag; } } }
        else { for (int i = 0; i < 2048; i += jump) {
#ifdef __aarch64__
                for (int j = 0; j < step; j += 4) { int curr = i + j; int k = curr + step; float32x4_t vr_curr = vld1q_f32(&real[curr]); float32x4_t vi_curr = vld1q_f32(&imag[curr]); float32x4_t vr_k = vld1q_f32(&real[k]); float32x4_t vi_k = vld1q_f32(&imag[k]); float tr_arr[4] = { twiddle_real_2048[(j+0)*twiddle_step], twiddle_real_2048[(j+1)*twiddle_step], twiddle_real_2048[(j+2)*twiddle_step], twiddle_real_2048[(j+3)*twiddle_step] }; float ti_arr[4] = { twiddle_imag_2048[(j+0)*twiddle_step], twiddle_imag_2048[(j+1)*twiddle_step], twiddle_imag_2048[(j+2)*twiddle_step], twiddle_imag_2048[(j+3)*twiddle_step] }; float32x4_t v_tr = vld1q_f32(tr_arr); float32x4_t v_ti = vld1q_f32(ti_arr); float32x4_t vt_real = vsubq_f32(vmulq_f32(vr_k, v_tr), vmulq_f32(vi_k, v_ti)); float32x4_t vt_imag = vaddq_f32(vmulq_f32(vr_k, v_ti), vmulq_f32(vi_k, v_tr)); vst1q_f32(&real[k], vsubq_f32(vr_curr, vt_real)); vst1q_f32(&imag[k], vsubq_f32(vi_curr, vt_imag)); vst1q_f32(&real[curr], vaddq_f32(vr_curr, vt_real)); vst1q_f32(&imag[curr], vaddq_f32(vi_curr, vt_imag)); }
#else
...
void custom_fft_1024_fixed(float* real, float* imag) {
    int j_idx = 0; for (int i = 0; i < 1023; i++) { if (i < j_idx) { float tr = real[i]; real[i] = real[j_idx]; real[j_idx] = tr; float ti = imag[i]; imag[i] = imag[j_idx]; imag[j_idx] = ti; } int k = 512; while (k <= j_idx) { j_idx -= k; k /= 2; } j_idx += k; }
...
void custom_fft_512_fixed(float* real, float* imag) {
    int j_idx = 0; for (int i = 0; i < 511; i++) { if (i < j_idx) { float tr = real[i]; real[i] = real[j_idx]; real[j_idx] = tr; float ti = imag[i]; imag[i] = imag[j_idx]; imag[j_idx] = ti; } int k = 256; while (k <= j_idx) { j_idx -= k; k /= 2; } j_idx += k; }
...
void custom_fft_256_fixed(float* real, float* imag) {
    int j_idx = 0; for (int i = 0; i < 255; i++) { if (i < j_idx) { float tr = real[i]; real[i] = real[j_idx]; real[j_idx] = tr; float ti = imag[i]; imag[i] = imag[j_idx]; imag[j_idx] = ti; } int k = 128; while (k <= j_idx) { j_idx -= k; k /= 2; } j_idx += k; }
...
void custom_fft_128_fixed(float* real, float* imag) {
    int j_idx = 0; for (int i = 0; i < 127; i++) { if (i < j_idx) { float tr = real[i]; real[i] = real[j_idx]; real[j_idx] = tr; float ti = imag[i]; imag[i] = imag[j_idx]; imag[j_idx] = ti; } int k = 64; while (k <= j_idx) { j_idx -= k; k /= 2; } j_idx += k; }
...
void custom_fft_64_fixed(float* real, float* imag) {
    int j_idx = 0; for (int i = 0; i < 63; i++) { if (i < j_idx) { float tr = real[i]; real[i] = real[j_idx]; real[j_idx] = tr; float ti = imag[i]; imag[i] = imag[j_idx]; imag[j_idx] = ti; } int k = 32; while (k <= j_idx) { j_idx -= k; k /= 2; } j_idx += k; }
...
void transpose_radar_cube(const float* src, float* dst, int n_samples) {
...
void execute_custom_pipeline(float* cube_real, float* cube_imag, float* trans_cust_r, float* trans_cust_i, int n_samples) {
    ...
    for (int ant = 0; ant < N_ANTENNAS; ant++) {
        for (int r = 0; r < n_samples; r++) {
            ...
            for (int chirp = 0; chirp < N_CHIRPS; chirp++) { 
                trans_cust_r[offset+chirp] *= win_256[chirp]; trans_cust_i[offset+chirp] *= win_256[chirp]; 
            }
            custom_fft_256_fixed(&trans_cust_r[offset], &trans_cust_i[offset]);
        }
    }
}
...
void execute_fftw_pipeline_optimized(fftwf_complex* cube, fftwf_plan p_range, fftwf_plan p_doppler, int n_samples) {
    ...
    for (int idx = 0; idx < total_elements; idx++) {
        ...
        cube[idx][0] *= win_256[chirp]; cube[idx][1] *= win_256[chirp];
    }
    for (int ant = 0; ant < N_ANTENNAS; ant++) {
...
void run_benchmark_for_size(int n_samples) {
    ...
    double target_R[] = {9.25, 22.40}; double target_v[] = {7.30, -15.80}; double target_theta[] = {-20.0, 35.0}; 
    for (int t_idx = 0; t_idx < 2; t_idx++) {
        double R = target_R[t_idx]; double v = target_v[t_idx]; double theta_rad = target_theta[t_idx] * PI / 180.0;
    ...
    for(int i=0; i<total_elements; i++) { 
        fftw_cube[i][0] = raw_real[i]; fftw_cube[i][1] = raw_imag[i]; 
    }
    ...
    for(int i=0; i<total_elements; i++) { 
        cust_cube_r[i] = raw_real[i]; cust_cube_i[i] = raw_imag[i]; 
        trans_cust_r[i] = 0; trans_cust_i[i] = 0; 
    }
    ...
    free(cust_cube_r); free(cust_cube_i); free(trans_cust_r); free(trans_cust_i);
    free(raw_real); free(raw_imag);
    malloc_trim(0);
