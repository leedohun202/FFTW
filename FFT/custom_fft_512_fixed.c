        ...
        if (step < 4) { 
            ...
            for (int i = 0; i < 512; i += jump) {
                ...
                for (int j = 0; j < step; j += 4) { 
                    ...
                    v_tr = vsetq_lane_f32(twiddle_real_512[(j+0)*twiddle_step], v_tr, 0);
                    v_tr = vsetq_lane_f32(twiddle_real_512[(j+1)*twiddle_step], v_tr, 1);
                    v_tr = vsetq_lane_f32(twiddle_real_512[(j+2)*twiddle_step], v_tr, 2);
                    v_tr = vsetq_lane_f32(twiddle_real_512[(j+3)*twiddle_step], v_tr, 3);
                    
                    float32x4_t v_ti = vdupq_n_f32(0.0f);
                    v_ti = vsetq_lane_f32(twiddle_imag_512[(j+0)*twiddle_step], v_ti, 0);
                    v_ti = vsetq_lane_f32(twiddle_imag_512[(j+1)*twiddle_step], v_ti, 1);
                    v_ti = vsetq_lane_f32(twiddle_imag_512[(j+2)*twiddle_step], v_ti, 2);
                    v_ti = vsetq_lane_f32(twiddle_imag_512[(j+3)*twiddle_step], v_ti, 3);
                ...
                for (int j = 0; j < step; j++) { 
                    int curr = i + j; int k = curr + step; 
                    float tr = twiddle_real_512[j * twiddle_step]; float ti = twiddle_imag_512[j * twiddle_step]; 
