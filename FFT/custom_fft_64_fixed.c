...
/**
 * @brief 64 포인트 1D 고속 푸리에 변환 (고정 크기 최적화)
 * @param real 실수부 데이터 배열 포인터
 * @param imag 허수부 데이터 배열 포인터
 * @details Angle(Azimuth) FFT 처리 등에 주로 사용되는 64 샘플 전용 함수입니다.
 */
#include "radar_fft.h"
        ...
        if (step < 4) { 
            ...
            for (int i = 0; i < 64; i += jump) {
                ...
                for (int j = 0; j < step; j += 4) { 
                    ...
                    v_tr = vsetq_lane_f32(twiddle_real_64[(j+0)*twiddle_step], v_tr, 0);
                    v_tr = vsetq_lane_f32(twiddle_real_64[(j+1)*twiddle_step], v_tr, 1);
                    v_tr = vsetq_lane_f32(twiddle_real_64[(j+2)*twiddle_step], v_tr, 2);
                    v_tr = vsetq_lane_f32(twiddle_real_64[(j+3)*twiddle_step], v_tr, 3);
                    
                    float32x4_t v_ti = vdupq_n_f32(0.0f);
                    v_ti = vsetq_lane_f32(twiddle_imag_64[(j+0)*twiddle_step], v_ti, 0);
                    v_ti = vsetq_lane_f32(twiddle_imag_64[(j+1)*twiddle_step], v_ti, 1);
                    v_ti = vsetq_lane_f32(twiddle_imag_64[(j+2)*twiddle_step], v_ti, 2);
                    v_ti = vsetq_lane_f32(twiddle_imag_64[(j+3)*twiddle_step], v_ti, 3);
