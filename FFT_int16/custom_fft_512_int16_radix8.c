...
/**
 * @brief 정밀 수술 완수 버전: Heap Corruption이 원천 차단된 512포인트 Radix-8 NEON 커널
 */
void custom_fft_512_int16_radix8(int16_t *__restrict__ r, int16_t *__restrict__ i) {
    const int N = 512;

    // [1단계] 고정소수점용 비트 리버설 (무결함 검증 패스)
    for (int k = 0; k < N; k++) {
    ...
    // [2단계] Radix-8 고속 NEON 나비 연산 스테이지
    for (int step = 8; step <= N; step *= 8) {
        int n8 = step >> 3; 
        int twiddle_stride = N / step;

        // 🛡️ [수술 포인트 A] j 루프의 점프 보폭과 경계를 n8과의 정렬 상태에 맞게 제어
        for (int j = 0; j < n8; j += 8) {
            ...
            for (int lane = 0; lane < 8; lane++) {
                ...
                // 트위들 가두리 양식장 적용
                c1[lane] = twiddle_int16_real_512[(idx1)     & 511]; s1[lane] = twiddle_int16_imag_512[(idx1)     & 511];
                c2[lane] = twiddle_int16_real_512[(idx1 * 2) & 511]; s2[lane] = twiddle_int16_imag_512[(idx1 * 2) & 511];
                c3[lane] = twiddle_int16_real_512[(idx1 * 3) & 511]; s3[lane] = twiddle_int16_imag_512[(idx1 * 3) & 511];
                c4[lane] = twiddle_int16_real_512[(idx1 * 4) & 511]; s4[lane] = twiddle_int16_imag_512[(idx1 * 4) & 511];
                c5[lane] = twiddle_int16_real_512[(idx1 * 5) & 511]; s5[lane] = twiddle_int16_imag_512[(idx1 * 5) & 511];
                c6[lane] = twiddle_int16_real_512[(idx1 * 6) & 511]; s6[lane] = twiddle_int16_imag_512[(idx1 * 6) & 511];
                c7[lane] = twiddle_int16_real_512[(idx1 * 7) & 511]; s7[lane] = twiddle_int16_imag_512[(idx1 * 7) & 511];
            }

#ifdef __aarch64__
            // 8-Lane NEON 트위들 레지스터 로드
            ...
            for (int blk = 0; blk < N; blk += step) {
                // 🛡️ [수술 포인트 B] 오프셋 임계 가드 장착
                // base 주소와 n8 보폭의 조합이 배열 최대 인덱스인 512 경계를 넘지 않도록 가드를 세웁니다.
                int base = blk + j;
                ...
                // 고정소수점 복소수 회전 나비 연산 수행 (Q15 fixed-point)
                ...
                int16x8_t v_out_r5 = vshrq_n_s16(vsubq_s16(vaddq_s16(r04_m, i26_m), vaddq_s16(r15_m, i37_m)), 3);
