        ...
        if (step < 8) { 
            for (int i = 0; i < N; i += jump) { 
                for (int j = 0; j < step; j++) { 
                    ...
                    int32_t tr = twiddle_int16_real_64[j * twiddle_step]; 
                    int32_t ti = twiddle_int16_imag_64[j * twiddle_step]; 
            ...
            for (int i = 0; i < N; i += jump) {
                for (int j = 0; j < step; j += 8) { 
                    ...
                    for (int lane = 0; lane < 8; lane++) {
                        tr_arr[lane] = twiddle_int16_real_64[(j + lane) * twiddle_step];
                        ti_arr[lane] = twiddle_int16_imag_64[(j + lane) * twiddle_step];
                    }
