...
extern float twiddle_real_512[];
extern float twiddle_imag_512[];

void custom_fft_512_radix4(float *__restrict__ r, float *__restrict__ i) {
    const int N = 512;
    ...
    for (int step = 8; step <= N; step <<= 2) {
        ...
        if (step < 16) {
            for (int j = 0; j < n4; j++) {
                ...
                float c1 = twiddle_real_512[idx1]; float s1 = twiddle_imag_512[idx1];
                float c2 = twiddle_real_512[idx2]; float s2 = twiddle_imag_512[idx2];
                float c3, s3;
                
                if (idx3 >= (N / 2)) {
                    c3 = -twiddle_real_512[idx3 - (N / 2)];
                    s3 = -twiddle_imag_512[idx3 - (N / 2)];
                } else {
                    c3 = twiddle_real_512[idx3];
                    s3 = twiddle_imag_512[idx3];
                }
            ...
            for (int j = 0; j < n4; j += 4) {
                ...
                for (int lane = 0; lane < 4; lane++) {
                    ...
                    c1_a[lane] = twiddle_real_512[idx1]; s1_a[lane] = twiddle_imag_512[idx1];
                    c2_a[lane] = twiddle_real_512[idx2]; s2_a[lane] = twiddle_imag_512[idx2];
                    if (idx3 >= (N / 2)) {
                        c3_a[lane] = -twiddle_real_512[idx3 - (N / 2)]; s3_a[lane] = -twiddle_imag_512[idx3 - (N / 2)];
                    } else {
                        c3_a[lane] = twiddle_real_512[idx3]; s3_a[lane] = twiddle_imag_512[idx3];
                    }
                }
            ...
            for (int j = 0; j < n4; j++) {
                ...
                float c3, s3;
                if (idx3 >= (N / 2)) {
                    c3 = -twiddle_real_512[idx3 - (N / 2)]; s3 = -twiddle_imag_512[idx3 - (N / 2)];
                } else {
                    c3 = twiddle_real_512[idx3]; s3 = twiddle_imag_512[idx3];
                }
