...
extern int bitrev_128[];
extern float twiddle_real_128[];
extern float twiddle_imag_128[];

void custom_fft_128_radix4(float *__restrict__ r, float *__restrict__ i) {
    const int N = 128;

    // 1. Bit-Reversal
    for (int k = 0; k < N; k++) {
        int rev = bitrev_128[k];
        if (k < rev) {
    ...
    // 3. 나머지 3개 스테이지: Radix-4 (step: 8, 32, 128)
    for (int step = 8; step <= N; step <<= 2) {
        int n4 = step >> 2; 
        int twiddle_stride = N / step; 

        // step이 8일 때는 n4=2 이므로 NEON 루프(4칸)를 돌릴 수 없어 C로 처리
        if (step < 16) {
            for (int j = 0; j < n4; j++) {
                ...
                float c1 = twiddle_real_128[idx1]; float s1 = twiddle_imag_128[idx1];
                float c2 = twiddle_real_128[idx2]; float s2 = twiddle_imag_128[idx2];
                float c3, s3;
                
                // 트위들 팩터는 N/2 크기까지만 저장되어 있으므로 오버플로우 방지
                if (idx3 >= (N / 2)) {
                    c3 = -twiddle_real_128[idx3 - (N / 2)];
                    s3 = -twiddle_imag_128[idx3 - (N / 2)];
                } else {
                    c3 = twiddle_real_128[idx3];
                    s3 = twiddle_imag_128[idx3];
                }
            ...
            for (int j = 0; j < n4; j += 4) {
                ...
                for (int lane = 0; lane < 4; lane++) {
                    ...
                    c1_a[lane] = twiddle_real_128[idx1]; s1_a[lane] = twiddle_imag_128[idx1];
                    c2_a[lane] = twiddle_real_128[idx2]; s2_a[lane] = twiddle_imag_128[idx2];
                    if (idx3 >= (N / 2)) {
                        c3_a[lane] = -twiddle_real_128[idx3 - (N / 2)]; s3_a[lane] = -twiddle_imag_128[idx3 - (N / 2)];
                    } else {
                        c3_a[lane] = twiddle_real_128[idx3]; s3_a[lane] = twiddle_imag_128[idx3];
                    }
                }
            ...
            for (int j = 0; j < n4; j++) {
                ...
                float c3, s3;
                if (idx3 >= (N / 2)) {
                    c3 = -twiddle_real_128[idx3 - (N / 2)]; s3 = -twiddle_imag_128[idx3 - (N / 2)];
                } else {
                    c3 = twiddle_real_128[idx3]; s3 = twiddle_imag_128[idx3];
                }
