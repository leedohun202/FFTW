...
void custom_fft_512_int16_radix4(int16_t *__restrict__ r, int16_t *__restrict__ i, int skip_shift) {
    const int N = 512;
    ...
    for (int step = 8; step <= N; step <<= 2) {
        ...
        if (step == 8) {
            int scalar_shift = (skip_shift == 1 && step >= 128) ? 0 : 2; 
            ...
            for (int j = 0; j < n4; j += 8) {
                ...
                for (int lane = 0; lane < 8; lane++) {
                    ...
                    c1_a[lane] = twiddle_int16_real_512[idx1]; s1_a[lane] = twiddle_int16_imag_512[idx1];
                    c2_a[lane] = twiddle_int16_real_512[idx2]; s2_a[lane] = twiddle_int16_imag_512[idx2];
                    if (idx3 >= (N / 2)) {
                ...
                for (int blk = 0; blk < N; blk += step) {
                    ...
                    if (skip_shift == 1 && step >= 128) {
                        // 스킵 구간은 시프트가 0이므로 반올림 불필요
