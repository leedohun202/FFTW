...
void custom_fft_256_int16_radix4(int16_t *__restrict__ r, int16_t *__restrict__ i) {
    const int N = 256;
    ...
    // 2. 순수 Radix-4 스테이지 (step: 4, 16, 64, 256)
    for (int step = 4; step <= N; step <<= 2) {
        int n4 = step >> 2; 
        int twiddle_stride = N / step; 

        // step이 4, 16일 때는 n4 < 8 이므로 C로 처리
        if (step < 32) {
            for (int j = 0; j < n4; j++) {
                ...
                if (idx3 >= (N / 2)) {
                    c3 = -twiddle_int16_real_256[idx3 - (N / 2)];
                    s3 = -twiddle_int16_imag_256[idx3 - (N / 2)];
                } else {
                    c3 = twiddle_int16_real_256[idx3];
                    s3 = twiddle_int16_imag_256[idx3];
                }
            ...
            for (int j = 0; j < n4; j += 8) {
                ...
                for (int lane = 0; lane < 8; lane++) {
                    ...
                    c1_a[lane] = twiddle_int16_real_256[idx1];
                    s1_a[lane] = twiddle_int16_imag_256[idx1];
                    c2_a[lane] = twiddle_int16_real_256[idx2];
                    s2_a[lane] = twiddle_int16_imag_256[idx2];

                    if (idx3 >= (N / 2)) {
                        c3_a[lane] = -twiddle_int16_real_256[idx3 - (N / 2)];
                        s3_a[lane] = -twiddle_int16_imag_256[idx3 - (N / 2)];
                    } else {
                        c3_a[lane] = twiddle_int16_real_256[idx3];
                        s3_a[lane] = twiddle_int16_imag_256[idx3];
                    }
                }
