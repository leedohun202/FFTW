...
void custom_fft_2048_int16_radix4(int16_t *__restrict__ r, int16_t *__restrict__ i) {
    ...
    // 2개의 데이터를 더하고 빼므로, 오버플로우 방지를 위해 >> 1 (2로 나누기) 수행
    for (int k = 0; k < N; k += 2) {
    ...
    // 3. 🟢 나머지 5개 스테이지: Radix-4 (step: 8, 32, 128, 512, 2048)
    for (int step = 8; step <= N; step <<= 2) {
        int n4 = step >> 2; 
        int twiddle_stride = N / step; 

        // step이 8일 때는 n4가 2이므로 NEON 벡터 루프(8칸)를 돌릴 수 없음.
        // 첫 Radix-4 스테이지는 순수 C로 처리
        if (step == 8) {
            for (int j = 0; j < n4; j++) {
...
#ifdef __aarch64__
            // 🚀 본격적인 NEON 가속 구간 (step 32, 128, 512, 2048)
