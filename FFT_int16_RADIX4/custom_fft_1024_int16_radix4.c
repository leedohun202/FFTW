...
extern int16_t twiddle_int16_real_1024[];
extern int16_t twiddle_int16_imag_1024[];
    ...
    for (int step = 4; step <= N; step <<= 2) {
        ...
        if (step == 4 || step == 16) {
            for (int j = 0; j < n4; j++) {
                ...
                int32_t c1 = twiddle_int16_real_1024[idx1]; int32_t s1 = twiddle_int16_imag_1024[idx1];
                int32_t c2 = twiddle_int16_real_1024[idx2]; int32_t s2 = twiddle_int16_imag_1024[idx2];
                int32_t c3, s3;
                
                if (idx3 >= (N / 2)) {
                    c3 = -twiddle_int16_real_1024[idx3 - (N / 2)];
                    s3 = -twiddle_int16_imag_1024[idx3 - (N / 2)];
                } else {
                    c3 = twiddle_int16_real_1024[idx3];
                    s3 = twiddle_int16_imag_1024[idx3];
                }
            ...
            for (int j = 0; j < n4; j += 8) {
                ...
                for (int lane = 0; lane < 8; lane++) {
                    ...
                    c1_a[lane] = twiddle_int16_real_1024[idx1];
                    s1_a[lane] = twiddle_int16_imag_1024[idx1];
                    c2_a[lane] = twiddle_int16_real_1024[idx2];
                    s2_a[lane] = twiddle_int16_imag_1024[idx2];

                    if (idx3 >= (N / 2)) {
                        c3_a[lane] = -twiddle_int16_real_1024[idx3 - (N / 2)];
                        s3_a[lane] = -twiddle_int16_imag_1024[idx3 - (N / 2)];
                    } else {
                        c3_a[lane] = twiddle_int16_real_1024[idx3];
                        s3_a[lane] = twiddle_int16_imag_1024[idx3];
                    }
                }
