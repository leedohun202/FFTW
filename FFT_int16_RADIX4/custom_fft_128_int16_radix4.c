...
extern int16_t twiddle_int16_real_128[];
extern int16_t twiddle_int16_imag_128[];

void custom_fft_128_int16_radix4(int16_t *__restrict__ r, int16_t *__restrict__ i) {
    const int N = 128;
    ...
    // 3. 나머지 스테이지: Radix-4 (step: 8, 32, 128)
    for (int step = 8; step <= N; step <<= 2) {
        int n4 = step >> 2; 
        int twiddle_stride = N / step; 

        if (step < 32) { // step 8은 n4=2이므로 C 처리
            for (int j = 0; j < n4; j++) {
                int idx1 = j * twiddle_stride;
                int idx2 = idx1 * 2;
                int idx3 = idx1 * 3;

                int32_t c1 = twiddle_int16_real_128[idx1]; int32_t s1 = twiddle_int16_imag_128[idx1];
                int32_t c2 = twiddle_int16_real_128[idx2]; int32_t s2 = twiddle_int16_imag_128[idx2];
                int32_t c3, s3;
                
                if (idx3 >= (N / 2)) {
                    c3 = -twiddle_int16_real_128[idx3 - (N / 2)];
                    s3 = -twiddle_int16_imag_128[idx3 - (N / 2)];
                } else {
                    c3 = twiddle_int16_real_128[idx3];
                    s3 = twiddle_int16_imag_128[idx3];
                }
            ...
            for (int j = 0; j < n4; j += 8) {
                ...
                for (int lane = 0; lane < 8; lane++) {
                    ...
                    c1_a[lane] = twiddle_int16_real_128[idx1]; s1_a[lane] = twiddle_int16_imag_128[idx1];
                    c2_a[lane] = twiddle_int16_real_128[idx2]; s2_a[lane] = twiddle_int16_imag_128[idx2];
                    if (idx3 >= (N / 2)) {
                        c3_a[lane] = -twiddle_int16_real_128[idx3 - (N / 2)];
                        s3_a[lane] = -twiddle_int16_imag_128[idx3 - (N / 2)];
                    } else {
                        c3_a[lane] = twiddle_int16_real_128[idx3];
                        s3_a[lane] = twiddle_int16_imag_128[idx3];
                    }
                }
